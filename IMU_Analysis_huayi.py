# 整合的IMU分析工具箱（华依 ARU8010A 二进制协议支持版）
...
# ===================================================================
# 华依 ARU8010A 二进制协议常量 (71 字节/帧)
# V0.7 手册定义的布局：
#   0-1: 报文头 "YS"
#   2-3: 帧序号 (uint16, 1..60000)
#   4: 数据长度 (uint8)
#   5: 温度 ID, 6: Flag0, 7-8: 温度 (int16, 0.01 °C)
#   9: 加速度 ID, 10: 长度, 11-22: 加速度 (3×int32, 1e-6 m/s²)
#   23: 陀螺 ID, 24: 长度, 25-36: 陀螺 (3×int32, 1e-6 °/s)
#   37: 欧拉角 ID, 38: Flag1, 39-50: 欧拉角 (3×int32, 1e-6 °)
#   51: 四元数 ID, 52: 长度, 53-68: 四元数 (4×int32, 1e-6)
#   69-70: 校验和
#   校验和为从 bytes[2] 到 bytes[68] 的逐字节双累加 (MOD 256)
# ===================================================================
FRAME_SIZE = 71
FRAME_SYNC_BYTES = b"YS"           # 同步字 0x59 0x53
FRAME_SYNC_OFFSET = 0
FRAME_SEQ_OFFSET = 2               # 帧序号 (uint16 LE)
FRAME_DATA_LEN_OFFSET = 4          # 数据长度 (uint8)
FRAME_ACC_OFFSET = 11              # 加速度计 X/Y/Z (3×int32 LE)
FRAME_GYRO_OFFSET = 25             # 陀螺仪 X/Y/Z (3×int32 LE)
FRAME_EULER_OFFSET = 39             # 欧拉角 pitch/roll/yaw (3×int32 LE)
FRAME_QUAT_OFFSET = 53             # 四元数 Q0/Q1/Q2/Q3 (4×int32 LE)
FRAME_TEMP_OFFSET = 7              # 温度 (int16 LE at offset 7, scale 0.01 -> °C)
FRAME_CHECKSUM_OFFSET = 69         # 校验和 (uint8×2)
FRAME_CHECK1_OFFSET = 69           # 校验和字节0
FRAME_CHECK2_OFFSET = 70           # 校验和字节1
FRAME_COUNTER_MIN = 1
...
_FMT_H_SIGNED = struct.Struct('<h')
_FMT_SENSOR = struct.Struct('<3i')
_FMT_QUAT = struct.Struct('<4i')


def _fletcher16(data: bytes) -> tuple:
    """Fletcher-16 校验和 (MOD 256, RFC 1146)
    ARU8010A 数据区: frame[2:69] (67字节)
    """
    s1, s2 = 0, 0
    for b in data:
        s1 = (s1 + b) & 0xFF
...
def _parse_one_frame(buf: bytes, verify_checksum: bool = True) -> dict | None:
    ...
    try:
        frame_seq = _FMT_H.unpack_from(buf, FRAME_SEQ_OFFSET)[0]
        data_len = buf[FRAME_DATA_LEN_OFFSET]
    except Exception:
        return None

    if verify_checksum:
        data_area = buf[FRAME_SEQ_OFFSET:FRAME_CHECK1_OFFSET]  # buf[2:69] 共67字节
        ck1_calc, ck2_calc = _fletcher16(data_area)
        ck1_file = buf[FRAME_CHECKSUM_OFFSET]
        ck2_file = buf[FRAME_CHECKSUM_OFFSET + 1]
        if ck1_calc != ck1_file or ck2_calc != ck2_file:
            return None

    try:
        acc_raw = _FMT_SENSOR.unpack_from(buf, FRAME_ACC_OFFSET)
        gyro_raw = _FMT_SENSOR.unpack_from(buf, FRAME_GYRO_OFFSET)
        euler_raw = _FMT_SENSOR.unpack_from(buf, FRAME_EULER_OFFSET)
        quat_raw = _FMT_QUAT.unpack_from(buf, FRAME_QUAT_OFFSET)
        temp_raw = _FMT_H_SIGNED.unpack_from(buf, FRAME_TEMP_OFFSET)[0]  # int16 LE
        # V0.7 已直接给出欧拉角；不再用未确认的四元数顺序反算。
        pitch, roll, yaw = (value * SCALE_EULER for value in euler_raw)
        q0, q1, q2, q3 = (value * SCALE_QUAT for value in quat_raw)
    except Exception:
        return None
...
class IMUDataAnalyzer:
    ...
    def _apply_trim(self):
        ...
        self.report_data["掐头去尾口径"] = (
            "按已收到的观测行数裁剪；若 frameCount 存在缺口，设置的分钟数与实际计数器时间可能不同，"
            "请同时参考按帧计数推定时长。"
        )
    ...
    def _update_frame_gap_statistics(self):
        ...
        if frame_col is None:
            self.frame_gap_stats = {
                "available": False,
                "reason": "未检测到 frameCount/帧序号列",
            }
            self.report_data["缺帧统计"] = "未检测到帧计数列，无法由序号估计缺帧"
        ...
        if self.binary_quality:
            ...
            checksum_state = "已启用" if self.binary_quality.get("binary_checksum_verified") else "未启用"
    ...
    def _load_csv_data(self):
        ...
        支持多种 CSV 格式：
        1. 华依通信协议dump格式：ax/ay/az/gx/gy/gz
        ...
        for old_name, new_name in HUAYI_CSV_COL_MAP.items():
    ...
    def extract_metadata(self):
        ...
        self.report_data["采样率来源"] = (
            "用户输入/程序配置；frameCount 只提供序号，不能单独确定采样频率"
        )
        ...
        if self.frame_gap_stats.get("available"):
            ...
            if np.isfinite(expected_s):
                self.report_data["按帧计数推定时长"] = f"{expected_s:.2f} 秒 ({expected_s/60:.2f} 分钟)"
            if np.isfinite(span_s):
                self.report_data["按帧计数首末跨度"] = f"{span_s:.2f} 秒"
        ...
        try:
            ...
            if not self.frame_gap_stats.get("available"):
                self.report_data["Allan缺帧策略"] = (
                    "未检测到有效 frameCount，无法由帧序号判断缺帧；Allan 输入仍按观测行处理。"
                )
            elif self.frame_gap_stats.get("gap_events", 0):
                self.report_data["Allan缺帧策略"] = (
                    "未重建缺失时间点；按观测行兼容计算，并在报告中单独列出 frameCount 缺口。"
                ...
                if uncertain:
                    ...
                    self.report_data["Allan缺帧策略"] = "未发现可由 frameCount 推断的正向缺口。"
                ...
                try:
                    ...
                    for j in range(len(taus_v)):
                        ...
                        curve_rows.append({
                            ...
                            "sample_rate_source": "user_config_framecounter_not_rate_verified",
    ...
    # ----------------------------------------------------------------
    # ----------------------------------------------------------------
    # 09 Allan 噪声分项拟合图（N/B/K/R，供应商风格；独立功能）
    # ----------------------------------------------------------------
    def plot_allan_noise_fit(self):
        ...
        try:
            ...
            fig, axes = plt.subplots(2, 3, figsize=(25, 13.5))
            fig.subplots_adjust(left=0.055, right=0.975, top=0.89, bottom=0.075,
            ...
            for i, col in enumerate(order):
                ...
                # 绘图单位与供应商图一致：陀螺 °/h，加计 μg。
                # ADEV 与 N/B/K/R 分项始终保持在同一绘图单位中。
                if is_gyro:
                    plot_factor = 3600.0
                    y = adev * plot_factor
                    y_label = r"$\sigma_A$ / (°/h)"
                ...
                # K/R：扣除 N、B 后，在 tau>=10 s 残差上识别斜率。
                # tau 使用秒，避免把绘图单位下的 N 错当成小时制 N。
                tail = (taus >= 10.0) & (ns >= 5)
                k_plot = np.nan
                r_plot = np.nan
                if np.count_nonzero(tail) >= 3 and np.isfinite(n_plot) and np.isfinite(b_plot):
                    ...
                    y_tail = y[tail]
                    resid2 = np.maximum(
                        y_tail ** 2 - n_plot ** 2 / t_s - (flicker * b_plot) ** 2,
                        0.0,
                    )
                    resid = np.sqrt(resid2)
                    valid = resid > 0
                    if np.count_nonzero(valid) >= 3:
                        sl, ic = np.polyfit(np.log10(t_s[valid]),
                                            np.log10(resid[valid]), 1)
                        if abs(sl - 0.5) <= 0.45:
                            k_plot = float(10 ** ic * np.sqrt(3.0))
                        elif abs(sl - 1.0) <= 0.45:
                            r_plot = float(10 ** ic * np.sqrt(2.0))

                # 合成拟合曲线：全部在绘图单位、tau 秒中计算。
                tgrid = np.logspace(np.log10(max(float(taus.min()), 1e-3)),
                                    np.log10(min(float(taus.max()), 1e3)), 400)
                fit2 = np.zeros_like(tgrid)
                if np.isfinite(n_plot):
                    fit2 += n_plot ** 2 / tgrid
                if np.isfinite(b_plot):
                    fit2 += (flicker * b_plot) ** 2
                if np.isfinite(k_plot):
                    fit2 += k_plot ** 2 * tgrid / 3.0
                if np.isfinite(r_plot):
                    fit2 += r_plot ** 2 * tgrid ** 2 / 2.0
                fit = np.sqrt(fit2)

                # 画图：供应商风格
                component_lines = {}
                component_colors = {"N": "#b06cc2", "B": "#8fbe55",
                                     "K": "#5da9e9", "R": "#d95f6a"}
                ax.loglog(tgrid, fit, color="#00e600", lw=2.2, label="AVAR fitted",
                          zorder=4)
                ax.loglog(taus, y, color="magenta", lw=1.8, marker="o", ms=4.2,
                          markerfacecolor="magenta", markeredgecolor="magenta",
                          label="AVAR original", zorder=5)
                if np.isfinite(n_plot):
                    component_lines["N"] = n_plot / np.sqrt(tgrid)
                if np.isfinite(b_plot):
                    component_lines["B"] = np.full_like(tgrid, flicker * b_plot)
                if np.isfinite(k_plot):
                    component_lines["K"] = k_plot * np.sqrt(tgrid / 3.0)
                if np.isfinite(r_plot):
                    component_lines["R"] = r_plot * tgrid / np.sqrt(2.0)
                for label, values in component_lines.items():
                    ax.loglog(tgrid, values, "--", color=component_colors[label],
                              lw=1.15, label=label, zorder=2)

                displayed = np.concatenate([
                    np.asarray(v, dtype=float).ravel()
                    for v in ([y, fit] + list(component_lines.values()))
                ])
                displayed = displayed[np.isfinite(displayed) & (displayed > 0)]
                if displayed.size:
                    ax.set_ylim(max(np.nanmin(displayed) * 0.55, 1e-12),
                                np.nanmax(displayed) * 1.8)
                ...
                # 参数文字框：仅对参数文字做单位换算；曲线本身不再二次换算。
                if is_gyro:
                    n_std = n_plot / 60.0 if np.isfinite(n_plot) else np.nan
                    b_std = b_plot if np.isfinite(b_plot) else np.nan
                    k_std = (k_plot * 60.0
                             if np.isfinite(k_plot) else np.nan)
                    r_std = (r_plot * 3600.0
                             if np.isfinite(r_plot) else np.nan)
                else:
                    n_std = (n_plot / plot_factor * 60.0
                             if np.isfinite(n_plot) else np.nan)
                    b_std = b_plot if np.isfinite(b_plot) else np.nan
                    ug_to_mmps2 = G0 * 1e-3
                    k_std = (k_plot * ug_to_mmps2 * 60.0
                             if np.isfinite(k_plot) else np.nan)
                    r_std = (r_plot * ug_to_mmps2 * 3600.0
                             if np.isfinite(r_plot) else np.nan)
                ...
                for label, values in component_lines.items():
                    y_right = float(values[-1])
                    if np.isfinite(y_right) and y_right > 0:
                        ax.text(1.006, y_right, label, transform=ax.get_yaxis_transform(),
                                fontsize=8.5, va="center", ha="left",
                                color=component_colors[label], clip_on=False)
                sensor_title = "%s%s" % (
                    ("陀螺仪" if is_gyro else "加速度计"), axis_cn[axname]
                )
                parameter_title = text.replace("\n", ";  ")
                ax.set_title(sensor_title + "\n" + parameter_title,
                             fontsize=7.8, fontweight="normal", pad=6)
...
"""
            ...
            hidden_report_keys = {
                ...
                "PSD图", "相关性图", "漂移分析图", "落点半径对比图", "落点半径对比数据", "Allan噪声分项拟合图", "Allan噪声分项拟合说明",
                "数据完整性图",
            }
...
class IMUAnalysisApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("IMU 数据分析工具箱 v2.0 (华依 ARU8010A 支持)")
        self.root.geometry("800x620")

        self.file_path = tk.StringVar()
        # 当前 ARU8010A 数据采集配置为 1000 Hz；其他档位仍可在下拉框中选择。
    ...
    def create_widgets(self):
        ...
        subtitle_label = ttk.Label(main_frame,
                                  text="华依 ARU8010A 一站式静态数据分析解决方案 (v2.0)",
        ...
        ttk.Label(sr_frame, text="（华依 ARU8010A 可选档位）",
